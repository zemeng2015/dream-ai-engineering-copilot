...
---
    ...
    ## Summary
    INC-102: OutputCollector retried after a transient storage error but did not use idempotency key. Duplicate result files were produced.
    ...
    ## Root Cause
    Collector retry path used artifact key only and did not include execution/task attempt idempotency.
...
- PR-508
    ...
    ## Regression Tests
    Add deterministic tests for the status, retry, output, permission, or preview branch that triggered INC-102.
