...
if (-not ($packetCheck -and $packetCheck.ok)) {
    Add-Action -Name "Generate final Devpost packet" -Reason "The copy/paste packet is still DRAFT." -Commands @(
        'scripts/qwencloud-finalize-after-urls.ps1 -DemoVideoUrl "<public-video-url>" -BackendUrl "<deployed-backend-url>"',
        'scripts/qwencloud-final-upload-bundle.ps1 -DemoVideoUrl "<public-video-url>" -BackendUrl "<deployed-backend-url>"'
    )
}
