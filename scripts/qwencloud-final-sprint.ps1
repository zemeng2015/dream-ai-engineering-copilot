...
if ($UseGitHubReleaseWorkflow -and $signals.githubReleaseWorkflowReady -and -not $signals.deployedBackendUrlPresent -and -not $signals.githubReleaseArtifactIngestReady) {
    Add-NextAction `
        ...
        -Reason "After the Qwen Cloud Release workflow finishes, the uploaded proof artifact must be downloaded locally so backend URL, showcase proof, and bundle hash can feed the final packet. The ingest command auto-selects the latest completed successful run and skips active runs." `
