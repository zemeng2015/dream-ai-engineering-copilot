...
function Is-PublicVideoUrl([string]$Url) {
    ...
    return [bool]($Url -match "^https?://((www|m|v)\.)?(youtube\.com|youtu\.be|vimeo\.com|facebook\.com|fb\.watch|youku\.com)/")
}
...
$md += @(
    ...
    "- Generate `scripts/qwencloud-cloud-credentials-handoff.ps1 -AllowDraft` for the local credential/setup template.",
    ...
    "- Upload `artifacts/qwencloud-proof/dream-qwencloud-devpost-final.mp4` using `docs/qwencloud-video-upload-handoff.md`, then paste the public YouTube, Vimeo, Facebook Video, fb.watch, or Youku URL.",
    ...
    "- Run `scripts/qwencloud-finalize-after-urls.ps1 -DemoVideoUrl <url> -BackendUrl <url>` as the final one-command gate before submitting Devpost.",
