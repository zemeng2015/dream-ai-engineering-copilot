...
export class MockDreamService {
  ...
  readonly health = {
    ...
    note: 'The Angular demo uses local mock memory data until API integration is enabled.',
  };
...
function relatedCodeForChangedFiles(changedFiles: string[]): CodebaseFile[] {
  ...
  const dfpMatches = DFP_PR_REVIEW_CODE_FILES.filter((file) => {
  ...
  return Array.from(new Map([...direct, ...conceptMatches, ...dfpMatches].map((file) => [file.id, file])).values()).slice(0, 6);
}

const DFP_PR_REVIEW_CODE_FILES: CodebaseFile[] = [
  {
    id: 'dfp-output-collector',
    path: 'examples/dfp-demo-repo/backend-api/src/main/java/com/democorp/dfp/output/OutputCollector.java',
    layer: 'backend',
    language: 'java',
    role: 'source',
    summary: 'Collects output artifacts and must reuse a stable idempotency key when storage retries return the same object.',
    concepts: ['output collection', 'idempotency', 'storage retry', 'duplicate output'],
    symbols: ['OutputCollector', 'collect', 'existsByIdempotencyKey'],
    relatedTests: ['OutputCollectorTest.java'],
  },
  {
    id: 'dfp-output-collector-test',
    path: 'examples/dfp-demo-repo/backend-api/src/test/java/com/democorp/dfp/output/OutputCollectorTest.java',
    layer: 'test',
    language: 'java',
    role: 'test',
    summary: 'Regression coverage proving duplicate storage retry does not create multiple output artifacts.',
    concepts: ['output collection', 'idempotency', 'duplicate output tests'],
    symbols: ['doesNotDuplicateArtifactsAfterStorageRetry'],
    relatedTests: [],
  },
];
...
const MOCK_CODEBASE_FILES: CodebaseFile[] = [
  {
    id: 'ecom-fe-app-component',
    path: 'ecom-product-discovery-ai/frontend-angular/src/app/app.component.ts',
    layer: 'frontend',
    language: 'typescript',
    role: 'source',
    summary: 'Angular workspace shell that coordinates collection, selection, review, image, supplier, export, and settings flows.',
    concepts: ['angular workspace', 'human review', 'selection workflow', 'image ops'],
    symbols: ['AppComponent', 'runBatchSelection', 'reviewCandidate', 'generateImages'],
    relatedTests: ['checkout-flow.test.mjs'],
  },
  {
    id: 'ecom-fe-api-service',
    path: 'ecom-product-discovery-ai/frontend-angular/src/app/api.service.ts',
    layer: 'frontend',
    language: 'typescript',
    role: 'source',
    summary: 'Angular API client for workflow, collection, supplier outreach, image operations, export, config, and license endpoints.',
    concepts: ['frontend api client', 'workflow api', 'collection api', 'license api'],
    symbols: ['ApiService', 'submitUploads', 'startCollectionJob', 'createStripeCheckoutSession'],
    relatedTests: [],
  },
  {
    id: 'ecom-fe-models',
    path: 'ecom-product-discovery-ai/frontend-angular/src/app/models.ts',
    layer: 'frontend',
    language: 'typescript',
    role: 'source',
    summary: 'Shared Angular interfaces for candidates, collection jobs, supplier campaigns, image pipeline, and export results.',
    concepts: ['frontend models', 'candidate state', 'collection job', 'image pipeline'],
    symbols: ['ProductCandidate', 'CollectionJob', 'ImagePipeline', 'ExportResult'],
    relatedTests: [],
  },
  {
    id: 'ecom-fe-checkout-flow',
    path: 'ecom-product-discovery-ai/frontend-angular/src/app/checkout-flow.ts',
    layer: 'frontend',
    language: 'typescript',
    role: 'source',
    summary: 'Checkout helpers normalize email-based user IDs, build return URLs, and protect local paid checkout flow assumptions.',
    concepts: ['checkout', 'license gate', 'idempotency', 'secret safety'],
    symbols: ['buildCheckoutReturnUrls', 'isStripeCheckoutUrl', 'buildCheckoutIdempotencyKey'],
    relatedTests: ['checkout-flow.test.mjs'],
  },
  {
    id: 'ecom-workflows-route',
    path: 'ecom-product-discovery-ai/app/api/routes/workflows.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'FastAPI workflow routes for submit, upload, bulk import, normalize, research, competitor discovery, score, review, and audit.',
    concepts: ['workflow api', 'bulk upload', 'normalization', 'selection scoring', 'human review'],
    symbols: ['submit_uploads', 'bulk_uploads', 'batch_select', 'score_candidate', 'review_candidate'],
    relatedTests: ['test_selection_and_images.py', 'test_authorization_gates.py'],
  },
  {
    id: 'ecom-collections-route',
    path: 'ecom-product-discovery-ai/app/api/routes/collections.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Collection routes for planning, URL validation, public source adapters, RPA recordings, collection jobs, and row import.',
    concepts: ['collection api', 'public source', 'url validation', 'rpa recording'],
    symbols: ['create_collection_plan', 'validate_collection_urls', 'create_collection_job', 'import_collection_job'],
    relatedTests: ['test_collection_api.py'],
  },
  {
    id: 'ecom-config-route',
    path: 'ecom-product-discovery-ai/app/api/routes/config.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Configuration routes expose safe public settings and user-level AI provider configuration without returning secrets.',
    concepts: ['ai config', 'public config', 'secret redaction', 'provider test'],
    symbols: ['get_public_config', 'get_user_ai_config', 'update_user_ai_config', 'test_user_ai_config'],
    relatedTests: ['test_config_api.py'],
  },
  {
    id: 'ecom-images-route',
    path: 'ecom-product-discovery-ai/app/api/routes/images.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Image routes create prompt briefs, optimize prompts, run image generation, manage provider mode, and record image reviews.',
    concepts: ['image prompt brief', 'image generation', 'provider mode', 'image review'],
    symbols: ['create_brief', 'generate_images_async', 'provider_status', 'review_variant'],
    relatedTests: ['test_selection_and_images.py', 'test_authorization_gates.py'],
  },
  {
    id: 'ecom-exports-route',
    path: 'ecom-product-discovery-ai/app/api/routes/exports.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Export routes create listing-readiness packages and serve access-bound package downloads.',
    concepts: ['export package', 'listing readiness', 'download gate', 'manifest'],
    symbols: ['export_candidate_package', 'download_export_artifact', '_assert_export_artifact_access'],
    relatedTests: ['test_authorization_gates.py'],
  },
  {
    id: 'ecom-license-route',
    path: 'ecom-product-discovery-ai/app/api/routes/license.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'License routes expose trial status, usage authorization, license redemption, admin grants, and checkout session creation.',
    concepts: ['license api', 'usage authorization', 'checkout', 'credits'],
    symbols: ['get_license_status', 'authorize_usage', 'create_stripe_checkout_session', 'grant_credits'],
    relatedTests: ['test_license_api.py'],
  },
  {
    id: 'ecom-supplier-route',
    path: 'ecom-product-discovery-ai/app/api/routes/supplier_outreach.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Supplier outreach routes create RFQ campaigns, list campaigns, send confirmed messages, and update manual quotes.',
    concepts: ['supplier outreach', 'RFQ', 'manual quote', 'campaign'],
    symbols: ['create_campaign', 'send_campaign', 'update_quote'],
    relatedTests: ['test_supplier_outreach.py'],
  },
  {
    id: 'ecom-intake-service',
    path: 'ecom-product-discovery-ai/app/services/intake.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Intake service stores uploaded artifacts, parses bulk rows, creates product candidates, and records source metadata.',
    concepts: ['intake', 'bulk row parser', 'artifact storage', 'product candidate'],
    symbols: ['IntakeService', 'submit_uploads', 'submit_bulk_uploads', '_candidate_from_bulk_row'],
    relatedTests: ['test_collection_api.py', 'test_selection_and_images.py'],
  },
  {
    id: 'ecom-document-extraction',
    path: 'ecom-product-discovery-ai/app/services/document_extraction.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Document preview extraction for text, JSON, CSV, XLSX, and PDF uploads used during candidate intake.',
    concepts: ['document extraction', 'upload preview', 'xlsx', 'pdf'],
    symbols: ['extract_preview', '_csv_preview', '_xlsx_preview', '_pdf_preview'],
    relatedTests: [],
  },
  {
    id: 'ecom-collection-service',
    path: 'ecom-product-discovery-ai/app/services/collection.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Collection service builds category plans, validates public URLs, creates source jobs, imports rows, and enforces public source safety.',
    concepts: ['collection plan', 'public url', 'source adapter', 'market profile'],
    symbols: ['CollectionService', 'create_plan', 'validate_public_urls', 'create_job', 'import_job'],
    relatedTests: ['test_collection_api.py'],
  },
  {
    id: 'ecom-source-safety',
    path: 'ecom-product-discovery-ai/app/services/source_safety.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Source safety helpers normalize domains, track cooldowns, and detect access-restricted public responses.',
    concepts: ['source safety', 'domain cooldown', 'access restriction', 'public fetch'],
    symbols: ['normalized_domain', 'domain_cooldown_reason', 'mark_domain_cooldown'],
    relatedTests: ['test_collection_api.py'],
  },
  {
    id: 'ecom-market-profiles',
    path: 'ecom-product-discovery-ai/app/services/market_profiles.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Market profile catalog selects region-specific source options, collection defaults, and image presets.',
    concepts: ['market profile', 'regional source', 'collection defaults', 'image preset'],
    symbols: ['list_market_profiles', 'get_market_profile', 'default_competitor_sources'],
    relatedTests: ['test_collection_api.py', 'test_config_api.py'],
  },
  {
    id: 'ecom-normalization-service',
    path: 'ecom-product-discovery-ai/app/services/normalization.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Normalization service converts messy notes and artifacts into structured candidate fields with deterministic fallback.',
    concepts: ['normalization', 'candidate fields', 'AI fallback', 'messy notes'],
    symbols: ['NormalizationService', 'normalize', '_apply_ai_payload', '_guess_title'],
    relatedTests: ['test_authorization_gates.py'],
  },
  {
    id: 'ecom-market-research-service',
    path: 'ecom-product-discovery-ai/app/services/market_research.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Market research service creates evidence plans and structured research prompts for manual or AI-assisted research.',
    concepts: ['market research', 'evidence plan', 'research prompt', 'evidence schema'],
    symbols: ['MarketResearchService', 'create_evidence_plan', 'submit_evidence', '_build_research_prompt'],
    relatedTests: ['test_competitor_discovery.py'],
  },
  {
    id: 'ecom-ai-client',
    path: 'ecom-product-discovery-ai/app/services/ai_client.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'AI client wraps chat calls, parses JSON output, captures usage, and estimates tokens for governed AI-assisted workflows.',
    concepts: ['AI client', 'usage capture', 'json parsing', 'token estimate'],
    symbols: ['AIClient', 'chat_json', '_parse_json_object', '_extract_usage'],
    relatedTests: ['test_authorization_gates.py'],
  },
  {
    id: 'ecom-competitor-discovery-service',
    path: 'ecom-product-discovery-ai/app/services/competitor_discovery.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Competitor discovery service collects bounded public evidence, extracts product signals, deduplicates results, and updates candidate scoring.',
    concepts: ['competitor discovery', 'public evidence', 'dedupe', 'score refresh'],
    symbols: ['CompetitorDiscoveryService', 'start_job', '_extract_without_ai', '_dedupe_evidence'],
    relatedTests: ['test_competitor_discovery.py'],
  },
  {
    id: 'ecom-selection-service',
    path: 'ecom-product-discovery-ai/app/services/selection.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Selection agent scores product opportunities, calculates profit signals, risk flags, missing fields, questions, and recommendation labels.',
    concepts: ['selection scoring', 'playbook', 'opportunity score', 'missing fields', 'risk flags'],
    symbols: ['SelectionAgentService', 'score_candidate', '_run_selection_playbook', '_dimension_score'],
    relatedTests: ['test_selection_and_images.py', 'test_authorization_gates.py'],
  },
  {
    id: 'ecom-matcher-service',
    path: 'ecom-product-discovery-ai/app/services/matcher.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Same-product matcher classifies exact, similar, and category competitor relationships for candidate evidence.',
    concepts: ['same product matcher', 'exact match', 'similar product', 'category competitor'],
    symbols: ['SameProductMatcher', 'classify'],
    relatedTests: ['test_competitor_discovery.py'],
  },
  {
    id: 'ecom-image-ops-service',
    path: 'ecom-product-discovery-ai/app/services/image_ops/service.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Image operations service manages prompt briefs, provider selection, generation jobs, variants, review tasks, and usage capture.',
    concepts: ['image ops', 'generation job', 'variant review', 'provider config'],
    symbols: ['ImageOpsService', 'create_prompt_brief', 'generate_images', 'review_variant'],
    relatedTests: ['test_selection_and_images.py', 'test_authorization_gates.py'],
  },
  {
    id: 'ecom-image-prompting',
    path: 'ecom-product-discovery-ai/app/services/image_ops/prompting.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Prompting helper builds image prompt briefs from candidate fields and public reference images.',
    concepts: ['image prompt brief', 'reference image', 'listing creative', 'prompt-only mode'],
    symbols: ['build_prompt_brief', '_reference_image_urls'],
    relatedTests: ['test_selection_and_images.py'],
  },
  {
    id: 'ecom-image-provider',
    path: 'ecom-product-discovery-ai/app/services/image_ops/provider.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Image provider protocol and request/response models keep external image generation behind a provider interface.',
    concepts: ['image provider', 'provider interface', 'adapter', 'external provider'],
    symbols: ['ImageProvider', 'ImageGenerationRequest', 'ImageGenerationResponse'],
    relatedTests: ['test_authorization_gates.py'],
  },
  {
    id: 'ecom-export-service',
    path: 'ecom-product-discovery-ai/app/services/export.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Export service creates listing-readiness markdown, JSON, CSV, manifests, and zip packages with safe artifact links.',
    concepts: ['export package', 'listing readiness', 'manifest', 'artifact access'],
    symbols: ['ExportService', 'export_candidate', '_readiness_preview', '_selection_export_summary'],
    relatedTests: ['test_authorization_gates.py'],
  },
  {
    id: 'ecom-license-service',
    path: 'ecom-product-discovery-ai/app/services/license.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'License service manages trial workspaces, plan features, credits, redemptions, manual grants, and usage authorization.',
    concepts: ['license gate', 'credits', 'feature access', 'usage authorization'],
    symbols: ['LicenseService', 'authorize_usage', 'consume_usage', 'redeem_code'],
    relatedTests: ['test_license_service.py', 'test_license_api.py'],
  },
  {
    id: 'ecom-billing-adapters',
    path: 'ecom-product-discovery-ai/app/services/billing_poc/adapters.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Billing provider protocols and facade define feature checks, usage commits, license activation, and manual payment grants.',
    concepts: ['billing adapter', 'provider interface', 'usage metering', 'manual payment'],
    symbols: ['BillingProviderAdapter', 'UsageMeteringProviderAdapter', 'MonetizationFacade'],
    relatedTests: ['test_billing_poc.py'],
  },
  {
    id: 'ecom-monetization-client',
    path: 'ecom-product-discovery-ai/app/services/monetization_core_client.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Remote monetization client maps product license, usage, checkout, and admin calls to the monetization-core API.',
    concepts: ['monetization core', 'remote billing', 'checkout', 'usage ledger'],
    symbols: ['MonetizationCoreClient', 'feature_authorization', 'create_checkout_session'],
    relatedTests: ['test_monetization_core_client.py'],
  },
  {
    id: 'ecom-supplier-service',
    path: 'ecom-product-discovery-ai/app/services/supplier_outreach.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Supplier outreach service drafts RFQ campaigns, discovers public emails, blocks unsafe sends, and records manual quotes.',
    concepts: ['supplier outreach', 'RFQ', 'email safety', 'manual quote'],
    symbols: ['SupplierOutreachService', 'create_campaign', 'send_campaign', '_rfq_copy'],
    relatedTests: ['test_supplier_outreach.py'],
  },
  {
    id: 'ecom-domain-models',
    path: 'ecom-product-discovery-ai/app/domain/models.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'Domain models for candidates, artifacts, evidence, image assets, review tasks, AI user config, and audit logs.',
    concepts: ['domain model', 'product candidate', 'audit log', 'review task'],
    symbols: ['ProductCandidate', 'InputArtifact', 'SourceEvidence', 'AuditLog'],
    relatedTests: ['test_selection_and_images.py'],
  },
  {
    id: 'ecom-sqlite-storage',
    path: 'ecom-product-discovery-ai/app/storage/sqlite.py',
    layer: 'python',
    language: 'python',
    role: 'source',
    summary: 'SQLite repository persists candidates, artifacts, image jobs, reviews, configs, and audit logs for local durable demos.',
    concepts: ['sqlite storage', 'local persistence', 'audit log', 'demo durability'],
    symbols: ['SQLiteRepository', 'save_candidate', 'list_audit_logs', 'save_ai_config'],
    relatedTests: ['test_license_service.py'],
  },
  {
    id: 'test-collection-api',
    path: 'ecom-product-discovery-ai/tests/test_collection_api.py',
    layer: 'test',
    language: 'python',
    role: 'test',
    summary: 'Tests collection planning, public source adapters, URL safety, row import, category catalog, and RPA recording boundaries.',
    concepts: ['collection tests', 'url safety', 'public source', 'row import'],
    symbols: ['test_collection_job_can_import_rows_into_existing_bulk_candidate_flow', 'test_collection_url_validation_respects_domain_cooldown'],
    relatedTests: [],
  },
  {
    id: 'test-competitor-discovery',
    path: 'ecom-product-discovery-ai/tests/test_competitor_discovery.py',
    layer: 'test',
    language: 'python',
    role: 'test',
    summary: 'Tests competitor discovery evidence import, China market default sources, and collection-to-rescore closed loop.',
    concepts: ['competitor discovery tests', 'evidence import', 'score refresh'],
    symbols: ['test_competitor_discovery_imports_evidence_and_refreshes_score'],
    relatedTests: [],
  },
  {
    id: 'test-selection-images',
    path: 'ecom-product-discovery-ai/tests/test_selection_and_images.py',
    layer: 'test',
    language: 'python',
    role: 'test',
    summary: 'Tests selection scoring, batch selection, review-preserved status, image brief gating, and public reference images.',
    concepts: ['selection tests', 'image tests', 'review gate', 'batch scoring'],
    symbols: ['test_batch_select_scores_candidates_with_synchronous_thread_pool', 'test_create_image_brief_returns_400_for_unapproved_candidate'],
    relatedTests: [],
  },
  {
    id: 'test-authorization-gates',
    path: 'ecom-product-discovery-ai/tests/test_authorization_gates.py',
    layer: 'test',
    language: 'python',
    role: 'test',
    summary: 'Tests AI usage authorization, image provider credit gates, export entitlements, and gated download safety.',
    concepts: ['authorization tests', 'credit gate', 'export gate', 'provider safety'],
    symbols: ['test_workflow_normalize_blocks_real_ai_when_license_denied', 'test_export_package_returns_gated_download_urls_bound_to_submitted_user'],
    relatedTests: [],
  },
  {
    id: 'test-license-api',
    path: 'ecom-product-discovery-ai/tests/test_license_api.py',
    layer: 'test',
    language: 'python',
    role: 'test',
    summary: 'Tests license status, redemption, manual payment, usage denial, checkout setup, and remote error hiding.',
    concepts: ['license tests', 'checkout tests', 'remote error hiding', 'admin secret'],
    symbols: ['test_license_api_manual_payment_and_status', 'test_license_api_checkout_returns_remote_checkout_url'],
    relatedTests: [],
  },
  {
    id: 'test-license-service',
    path: 'ecom-product-discovery-ai/tests/test_license_service.py',
    layer: 'test',
    language: 'python',
    role: 'test',
    summary: 'Tests trial seeding, plan features, credits, redemption limits, enforce mode, and corrupt state handling.',
    concepts: ['license service tests', 'trial workspace', 'credits', 'enforce mode'],
    symbols: ['test_default_trial_workspace_is_seeded', 'test_enforce_mode_blocks_missing_feature_and_insufficient_credits'],
    relatedTests: [],
  },
  {
    id: 'test-monetization-core',
    path: 'ecom-product-discovery-ai/tests/test_monetization_core_client.py',
    layer: 'test',
    language: 'python',
    role: 'test',
    summary: 'Tests remote monetization manual payments, feature authorization, usage summaries, idempotency, and checkout mapping.',
    concepts: ['monetization tests', 'remote billing', 'usage ledger', 'checkout mapping'],
    symbols: ['test_remote_manual_payment_and_feature_authorization', 'test_remote_checkout_session_maps_to_core_checkout_api'],
    relatedTests: [],
  },
  {
    id: 'test-supplier-outreach',
    path: 'ecom-product-discovery-ai/tests/test_supplier_outreach.py',
    layer: 'test',
    language: 'python',
    role: 'test',
    summary: 'Tests supplier outreach campaign send blocking and manual quote updates.',
    concepts: ['supplier tests', 'send block', 'manual quote', 'RFQ'],
    symbols: ['test_supplier_outreach_campaign_send_block_and_manual_quote'],
    relatedTests: [],
  },
  {
    id: 'test-fe-checkout-flow',
    path: 'ecom-product-discovery-ai/frontend-angular/scripts/checkout-flow.test.mjs',
    layer: 'test',
    language: 'typescript',
    role: 'test',
    summary: 'Frontend checkout helper tests cover return result parsing, user ID normalization, and idempotency key generation.',
    concepts: ['frontend checkout tests', 'idempotency', 'return url', 'user id'],
    symbols: ['normalizeCheckoutEmail', 'buildCheckoutReturnUrls', 'buildCheckoutIdempotencyKey'],
    relatedTests: [],
  },
];
...
const MOCK_SCORECARDS: EvaluationScorecard[] = [
  ...
  createScorecard({
    ...
    sourcePaths: [
      ...
      'examples/dfp-demo-repo/backend-api/src/test/java/com/democorp/dfp/output/OutputCollectorTest.java',
    ],
    recommendations: [
      'Confirm retry/idempotency boundary with backend reviewer.',
      'Keep duplicate-output regression tests linked before PR comment handoff.',
    ],
  }),
];
